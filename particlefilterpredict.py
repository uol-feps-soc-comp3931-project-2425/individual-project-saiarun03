...
def initialize_particle_filter(num_particles, frame_width, frame_height):
    # Initialize particles with position and velocity
    particles = np.zeros((num_particles, 4))  # [x, y, vx, vy]
    particles[:, 0] = np.random.uniform(0, frame_width, num_particles)  # x
    particles[:, 1] = np.random.uniform(0, frame_height, num_particles)  # y
    particles[:, 2] = np.random.normal(0, 1, num_particles)  # vx
    particles[:, 3] = np.random.normal(0, 1, num_particles)  # vy
    weights = np.ones(num_particles) / num_particles
    return particles, weights

def predict_particles(particles, motion_noise=1):
    # Constant velocity model with noise
    particles[:, 0] += particles[:, 2] + np.random.normal(0, motion_noise, particles.shape[0])  # x += vx
    particles[:, 1] += particles[:, 3] + np.random.normal(0, motion_noise, particles.shape[0])  # y += vy
    return particles

def update_weights(particles, weights, measurement, measurement_noise=10):
    # Update weights based on how close particles are to the measurement
    likelihood = multivariate_normal(measurement, cov=measurement_noise).pdf(particles[:, :2])
    weights *= likelihood
...
def systematic_resample(weights):
    ...
    for i in range(num_particles):
        indices[i] = np.searchsorted(cumulative_sum, u + i * step)
    return indices
...
num_particles = 200  # Increased number of particles for better accuracy
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
particles, weights = initialize_particle_filter(num_particles, frame_width, frame_height)
    ...
    if ret:
        ...
        if len(box) != 0:
            for i in range(rows):
                ...
                weights = update_weights(particles, weights, measurement)
                particles, weights = resample_particles(particles, weights)
                
                # Estimate ball position as the mean of the particles
                predicted_x, predicted_y = np.mean(particles[:, :2], axis=0)
                predicted_positions.append((predicted_x, predicted_y))
                actual_positions.append((centroid_x, centroid_y))
                
                # Draw particles
                for particle in particles:
                    cv2.circle(frame, tuple(particle[:2].astype(int)), radius=1, color=(0, 255, 255), thickness=-1)
                
                # Draw predicted position
                cv2.circle(frame, (int(predicted_x), int(predicted_y)), radius=3, color=(0, 255, 0), thickness=-1)
